#!/usr/bin/env bash
# Strict clean-allocation audit for C1 value-shard P1 versus PREFETCH=2.
set -Eeuo pipefail
...
sha256sum \
  ...
  "$OWNED/run_prefetch2_final.py" "$OWNED/run_clean_prefetch2_audit.sh" \
...
"$PYTHON" "$RUNNER" --reference-root "$REFERENCE_ROOT" --T 256 --small-heads 1,2,4 \
  --states all --no-bench --json "$LOG_DIR/c1_prefetch2_${LABEL}_small_matrix.json"
require_clean BETWEEN_EXACT_AND_BENCH || exit 93

echo "===== H64_BF16_P1_P2_ABBA ====="
"$PYTHON" "$RUNNER" --reference-root "$REFERENCE_ROOT" --T 8192 --H 64 --states bf16 \
  --warmup 30 --iters 200 --repeats 5 \
  --json "$LOG_DIR/c1_prefetch2_${LABEL}_h64_bf16.json"
