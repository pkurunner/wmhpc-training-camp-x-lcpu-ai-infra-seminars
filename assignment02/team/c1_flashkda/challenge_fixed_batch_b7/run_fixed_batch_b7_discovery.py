#!/usr/bin/env python3
"""Fail-closed, discovery-only B=7 fixed-batch raw-wrapper experiment.
...
CLEAN_GPU_GATE_ENV = "C1_FIXED_BATCH_B7_DISCOVERY_CLEAN_GPU"
...
def _case() -> shared.Case:
    return shared.Case(
        ...
        family="fixed_batch_b7_discovery_only",
    )
...
def _initial_result(args: argparse.Namespace) -> dict[str, object]:
    return {
        "schema_version": 1,
        "purpose": (
            "B=7 fixed-batch raw-wrapper discovery only; direct calls only, no dispatcher/map "
...
def main() -> None:
    ...
    if args.describe:
        ...
        print(f"wrote B=7 discovery description {args.json}")
        return

    if os.environ.get(CLEAN_GPU_GATE_ENV) != "1":
        raise RuntimeError(
            "refusing a direct GPU run: use run_clean_fixed_batch_b7_discovery.sh so the "
    ...
    for contract_index, contract in enumerate(RAW_CONTRACTS):
        print(f"B=7 raw ABI exactness {contract}")
    ...
    for contract_index, contract in enumerate(PERFORMANCE_CONTRACTS):
        ...
        for repeat_index in range(REPEATS_PER_PROCESS):
            ...
            print(
                f"B=7 direct cyclic benchmark process={args.process_index} "
    ...
    _write(args.json, result)
    print(f"wrote B=7 discovery artifact {args.json}; discovery-only/no-release-authority")
