#!/usr/bin/env python3
"""Independent stdlib-only audit for B=7 fixed-batch discovery artifacts.
...
def main() -> None:
    ...
    parser.add_argument("main_json", nargs=2, type=Path, help="the two fresh B=7 main-process artifacts")
    ...
    decision = (
        "eligible_for_independent_confirmation_only"
        if all_eligible
        else "baseline_stop_no_second_allocation"
    )
    payload: dict[str, object] = {
        "schema_version": 1,
        "purpose": "independent B=7 discovery audit; no production mapping or release authority",
        ...
        "second_allocation_decision": {
            ...
            "negative_action": "baseline_stop_no_second_allocation",
            "automatic_second_allocation_submitted": False,
        },
        "complete": True,
    }
    atomic_write(args.json, payload)
    print(f"wrote independent B=7 discovery audit {args.json}; {decision}")
