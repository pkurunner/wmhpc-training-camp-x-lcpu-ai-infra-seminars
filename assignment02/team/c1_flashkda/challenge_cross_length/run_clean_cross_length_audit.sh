#!/usr/bin/env bash
# Clean B300 cross-length/head/state audit.
set -Eeuo pipefail

if [[ "${C1_CROSS_LENGTH_GPU_AUTHORIZED:-0}" != 1 || "${1:-}" != "--authorized-by-parent" ]]; then
    echo "refusing GPU run: set C1_CROSS_LENGTH_GPU_AUTHORIZED=1 and pass --authorized-by-parent" >&2
    exit 64
fi
shift
...
OWNED="$A02_ROOT/team/c1_flashkda/challenge_cross_length"
RESULTS_DIR="${RESULTS_DIR:-$OWNED/results}"
mkdir -p "$RESULTS_DIR"
LOG="$RESULTS_DIR/c1_cross_length_${LABEL}_job${SLURM_JOB_ID:-none}.log"
exec > >(tee "$LOG") 2>&1
...
sha256sum \
    "$OWNED/run_cross_length.py" \
    "$OWNED/run_clean_cross_length_audit.sh" \
...
cd "$PATCHED_ROOT"
echo "===== CROSS_LENGTH_HEAD_STATE ====="
"$PYTHON_BIN" "$OWNED/run_cross_length.py" --warmup 30 --samples 300 \
    --json "$RESULTS_DIR/c1_cross_length_${LABEL}.json"
require_clean AFTER_SWEEP || exit 93
