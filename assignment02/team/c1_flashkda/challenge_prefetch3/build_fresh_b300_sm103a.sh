#!/usr/bin/env bash
# CPU-only fresh build inside the existing B300 allocation; never imports/runs the extension.
set -Eeuo pipefail

SOURCE_BASE="${SOURCE_BASE:-/home/lcpu/85117379/flashkda-1ce47ea}"
SOURCE_FRESH="${SOURCE_FRESH:-/home/lcpu/85117379/flashkda-prefetch3-1ce47ea-b300-r1}"
...
SUPPORT_BIN="${SUPPORT_BIN:-/home/lcpu/85117379/c1-prefetch3-support-b300-r1/bin}"
BUILD_LOG="${BUILD_LOG:-/home/lcpu/85117379/c1_prefetch3_build_b300_r1.log}"
...
for required in "$SOURCE_BASE/.git" "$SOURCE_BASE/cutlass/include/cutlass/cutlass.h" \
                "$A02_ROOT/team/c1_flashkda/challenge_prefetch3/apply_prefetch3_patch.py" \
                "$A02_ROOT/team/c1_flashkda/challenge_prefetch3/provisioning_git_shim.sh" \
...
mkdir -p "$SUPPORT_BIN"
cp "$A02_ROOT/team/c1_flashkda/challenge_prefetch3/provisioning_git_shim.sh" "$SUPPORT_BIN/git"
chmod 755 "$SUPPORT_BIN/git"
exec > >(tee "$BUILD_LOG") 2>&1
date -Is
hostname
printf 'SLURM_JOB_ID=%s CUDA_VISIBLE_DEVICES=%s\n' "$SLURM_JOB_ID" "${CUDA_VISIBLE_DEVICES:-unset}"
[[ -z "${CUDA_VISIBLE_DEVICES:-}" ]] || { echo "build requires CUDA_VISIBLE_DEVICES empty" >&2; exit 68; }
sha256sum "$A02_ROOT/team/c1_flashkda/challenge_prefetch3/apply_prefetch3_patch.py"
...
GENERATOR="$A02_ROOT/team/c1_flashkda/challenge_prefetch3/apply_prefetch3_patch.py"
...
sha256sum flash_kda_C.cpython-312-x86_64-linux-gnu.so \
  ...
  csrc/smxx/fwd_kernel2_vshard_p3.cuh csrc/smxx/fwd_launch.cu
git status --short
date -Is
