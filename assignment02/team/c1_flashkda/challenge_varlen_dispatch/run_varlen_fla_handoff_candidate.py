#!/usr/bin/env python3
"""Diagnostic-only B300 public-FLA packed-varlen handoff candidate gate.

Production policy is never edited: this runner first proves the frozen r5
two-cell map, then temporarily installs the frozen r4 ten-cell candidate map
only in this Python process, and restores the *same* production map object in
``finally``.  Its result has no release authority.
"""
...
CLEAN_GPU_GATE_ENV = "C1_VARLEN_FLA_HANDOFF_CANDIDATE_CLEAN_GPU"
RUNNER_SHA256_ENV = "C1_VARLEN_FLA_HANDOFF_CANDIDATE_RUNNER_SHA256"
...
# The production whitelist is deliberately the r5 public-performance release
# intersection.  This runner refuses to begin unless its contents are still
# exactly these two cells.  The broader map below is an in-process diagnostic
# candidate only; it is never assigned as a new object or written to disk.
...
CANDIDATE_PUBLIC_VARIANTS = {
    (_OFFSETS_BY_CASE["equal_n2_h12_t2048"], "none"): "vshard4_p2",
    (_OFFSETS_BY_CASE["equal_n2_h12_t2048"], "fp32_final_only"): "vshard4_p2",
    (_OFFSETS_BY_CASE["equal_n2_h12_t2048"], "fp32_both"): "vshard4_p2",
    (_OFFSETS_BY_CASE["equal_n4_h12_t2048"], "none"): "vshard2_p2",
    (_OFFSETS_BY_CASE["equal_n4_h12_t2048"], "fp32_final_only"): "vshard2_p2",
    (_OFFSETS_BY_CASE["mixed_n6_h12_t8192"], "none"): "vshard2_p2",
    (_OFFSETS_BY_CASE["mixed_n6_h12_t8192"], "fp32_final_only"): "vshard2_p2",
    (_OFFSETS_BY_CASE["skew_n6_h12_t12288"], "none"): "vshard2_p2",
    (_OFFSETS_BY_CASE["skew_n6_h12_t12288"], "fp32_final_only"): "vshard2_p2",
    (_OFFSETS_BY_CASE["skew_n6_h12_t12288"], "fp32_both"): "vshard4_p2",
}
...
POSITIVE_CELLS = tuple(
    Cell(case, contract, CANDIDATE_PUBLIC_VARIANTS[(_OFFSETS_BY_CASE[case.name], contract)])
    for case in confirmation.CASES
    for contract in confirmation.FLA_PUBLIC_CONTRACTS
    if (_OFFSETS_BY_CASE[case.name], contract) in CANDIDATE_PUBLIC_VARIANTS
)
POLICY_FALLBACK_CELLS = tuple(
    Cell(case, contract, None)
    for case in confirmation.CASES
    for contract in confirmation.FLA_PUBLIC_CONTRACTS
    if (_OFFSETS_BY_CASE[case.name], contract) not in CANDIDATE_PUBLIC_VARIANTS
)
...
class _StateMeta:
    """Torch-free state metadata used only to prove map lookup behavior."""
    ...
    def __init__(self, sequences: int) -> None:
        self.shape = (sequences, HEADS, DIM, DIM)

    def is_contiguous(self) -> bool:
        return True


def _varlen_contract_metas(offsets: tuple[int, ...], contract: str) -> tuple[object | None, object | None]:
    sequences = len(offsets) - 1
    if contract == "none":
        return None, None
    if contract == "fp32_final_only":
        return None, _StateMeta(sequences)
    if contract == "fp32_both":
        return _StateMeta(sequences), _StateMeta(sequences)
    raise AssertionError(f"unknown candidate contract {contract!r}")


def _assert_map_values_and_behavior(expected: Mapping[tuple[tuple[int, ...], str], str], label: str) -> dict[str, object]:
    """Validate mutable map contents and its real public selection semantics."""

    actual = auto_dispatch._VARLEN_PUBLIC_VARIANTS
    if dict(actual) != dict(expected):
        raise RuntimeError(f"{label}: _VARLEN_PUBLIC_VARIANTS contents drift")
    decisions: dict[str, object] = {}
    for (offsets, contract), variant in expected.items():
        initial, final = _varlen_contract_metas(offsets, contract)
        selected, reason = auto_dispatch.varlen_public_variant(offsets, HEADS, initial, final, 0)
        if selected != variant or not reason.endswith("_whitelist_hit"):
            raise RuntimeError(
                f"{label}: public map behavior drift for {offsets}/{contract}: "
                f"selected={selected!r}, reason={reason!r}"
            )
        decisions[f"{','.join(map(str, offsets))}/{contract}"] = {
            "selected_variant": selected,
            "reason": reason,
        }
    return {
        "map_object_id": id(actual),
        "entries": len(actual),
        "behavior": decisions,
        "passed": True,
    }
...
def _install_candidate_map() -> tuple[dict[tuple[tuple[int, ...], str], str], dict[str, object]]:
    """Mutate only the existing process-local map object after r5 proof."""
    ...
    if dict(original) != expected_production:
        raise RuntimeError("production varlen map is not exactly the frozen r5 two-cell map")
    production_behavior = _assert_map_values_and_behavior(expected_production, "production-before-candidate")
    original.clear()
    original.update(CANDIDATE_PUBLIC_VARIANTS)
    if auto_dispatch._VARLEN_PUBLIC_VARIANTS is not original:
        raise RuntimeError("candidate map installation replaced production map object")
    candidate_behavior = _assert_map_values_and_behavior(CANDIDATE_PUBLIC_VARIANTS, "candidate-installed")
    return expected_production, {
        "production_map_object_id": id(original),
        "production_before": production_behavior,
        "candidate_installed": candidate_behavior,
        "passed": True,
    }


def _restore_production_map(
    original: object,
    production_expected: Mapping[tuple[tuple[int, ...], str], str],
) -> dict[str, object]:
    """Restore contents on the original object and prove lookup behavior again."""

    if auto_dispatch._VARLEN_PUBLIC_VARIANTS is not original:
        raise RuntimeError("cannot restore: candidate code replaced the production map object")
    if not isinstance(original, dict):
        raise RuntimeError("cannot restore: original production map object is not mutable")
    original.clear()
    original.update(production_expected)
    if auto_dispatch._VARLEN_PUBLIC_VARIANTS is not original:
        raise RuntimeError("production map object identity changed during restoration")
    behavior = _assert_map_values_and_behavior(production_expected, "production-restored")
    rejected_initial, rejected_final = _varlen_contract_metas(_OFFSETS_BY_CASE["equal_n2_h12_t2048"], "none")
    rejected, reason = auto_dispatch.varlen_public_variant(
        _OFFSETS_BY_CASE["equal_n2_h12_t2048"], HEADS, rejected_initial, rejected_final, 0
    )
    if rejected is not None or not reason.endswith("_not_whitelisted"):
        raise RuntimeError("production-restored: candidate equal-N2 cell remains custom-eligible")
    return {
        "map_object_id": id(original),
        "entries": len(original),
        "behavior": behavior,
        "candidate_cell_after_restore": {"selected_variant": rejected, "reason": reason},
        "passed": True,
    }


def _capture_prepare_spy_state(backend: object) -> tuple[bool, object | None, object]:
    """Capture both instance shadow state and the normally resolved descriptor."""
...
def _restore_prepare_spy_state(
    ...
    *,
    ...
    """Undo a temporary instance spy without retaining a bound-method shadow."""
...
def _assert_no_prepare_instance_shadow(backend: object, label: str) -> dict[str, object]:
    """Fail closed if an instrumentation wrapper remains before performance."""
...
def _initial_result(args: argparse.Namespace) -> dict[str, object]:
    return {
        "schema_version": 1,
        "purpose": "diagnostic-only public FLA packed-varlen C1 handoff candidate gate; no production policy mutation",
        ...
        "performance": {
            "pre_registered": {
                ...
                "candidate_rule": "A candidate cell would pass only if both repeats have C1 as P50/P95/P99 winner with every margin >=2%; this diagnostic never changes a release policy.",
        ...
        "failed_cells": [],
        "candidate_map": {
            "production_expected": dict(PRODUCTION_PUBLIC_VARIANTS),
            "candidate_cells": [
                {"offsets": list(offsets), "contract": contract, "variant": variant}
                for (offsets, contract), variant in CANDIDATE_PUBLIC_VARIANTS.items()
            ],
            "installed": False,
            "restored": False,
        },
        "gates": {
            "scope": {
                ...
                "passed": len(POSITIVE_CELLS) == 10 and len(POLICY_FALLBACK_CELLS) == 2,
            },
...
def _candidate_public_handoff_probe(
    cell: Cell,
    x: object,
    ...
    """Prove one verifier-side prepare is consumed by the matching public body.

    The private-method wrapper exists only around these non-timed correctness
    calls.  All timing retains merely the existing public registry route
    counters, so it cannot measure this Python-level spy.
    """
...
def _positive_cell(cell: Cell, x: object, cpu: torch.Tensor, gpu: torch.Tensor, originals: Mapping[str, Callable[..., Any]], counts: Mapping[str, int], public_fn: Callable[..., Any], c1: object, pinned: object, torch_ref: Callable[..., None], seed: int) -> dict[str, object]:
    ...
    with torch.inference_mode():
        ...
        (
            ...
            public_pinned_spy,
            handoff_pinned,
        ) = _candidate_public_handoff_probe(
            cell, x, initial, final, gpu, cpu, public_fn, c1, counts
        )
...
def _cold_c1_observation(cell: Cell, x: object, cpu: torch.Tensor, gpu: torch.Tensor, public_fn: Callable[..., Any], counts: Mapping[str, int]) -> dict[str, object]:
    """A single un-gated cold cache observation, outside all timed samples."""
    import torch
...
def _candidate_performance_gate(public_fn: Callable[..., Any], counts: Mapping[str, int], c1: object, result: dict[str, object], json_path: Path) -> None:
    """Measure every candidate cell; results remain diagnostic-only would-pass facts."""
    import torch
    ...
    shadow_gate = _assert_no_prepare_instance_shadow(c1, "before candidate performance")
    gates = result["gates"]
    if not isinstance(gates, dict):
        raise TypeError("candidate gate result schema corrupted")
    ...
    try:
        ...
        for cell_index, cell in enumerate(POSITIVE_CELLS):
            ...
            would_pass = all(bool(repeat.get("repeat_gate_pass")) for repeat in repeats)
            cell_record["would_pass"] = would_pass
            performance["cells"][cell.key] = cell_record  # type: ignore[index]
            if would_pass:
                performance["would_pass_cells"].append(cell.key)  # type: ignore[index]
                result["would_pass_cells"].append(cell.key)  # type: ignore[index]
            else:
                performance["failed_cells"].append(cell.key)  # type: ignore[index]
                result["failed_cells"].append(cell.key)  # type: ignore[index]
            _write(json_path, result)
    finally:
        _restore_env("C1_B300_FLASH_KDA", previous_c1)
        _restore_env("FLA_FLASH_KDA", previous_pinned)
    performance["complete"] = True
    if result["would_pass_cells"] != performance["would_pass_cells"] or result["failed_cells"] != performance["failed_cells"]:
        raise AssertionError("top-level candidate performance mapping drift")
...
def main() -> None:
    ...
    if len(POSITIVE_CELLS) != 10 or positive_keys != expected_candidate_keys:
        raise RuntimeError("candidate varlen scope must be exactly the frozen r4 ten-cell map")
    if (
        len(POLICY_FALLBACK_CELLS) != 2
        ...
        raise RuntimeError("candidate fallback scope must be exactly the two non-candidate public cells")
    ...
    if (
        ...
        raise RuntimeError("candidate fallback classifications must be the frozen raw/record-only pair")
    result = _initial_result(args)
    if args.describe:
        result["describe_only"] = True
        _write(args.json, result)
        print(f"wrote packed-varlen handoff candidate plan {args.json}")
        return
    if args.cpu_construction_check:
        result["cpu_construction_check"] = _cpu_construction_check()
        result["cpu_only"] = True
        _write(args.json, result)
        print(f"wrote packed-varlen handoff candidate CPU construction check {args.json}")
        return
    ...
    production_map = auto_dispatch._VARLEN_PUBLIC_VARIANTS
    production_snapshot = dict(production_map)
    try:
        production_expected, install_evidence = _install_candidate_map()
        if production_expected != production_snapshot:
            raise RuntimeError("production snapshot drifted before candidate install")
        candidate_map_result = result["candidate_map"]
        if not isinstance(candidate_map_result, dict):
            raise TypeError("candidate map result schema corrupted")
        candidate_map_result["install"] = install_evidence
        candidate_map_result["installed"] = True
        _write(args.json, result)
        ...
        with torch.inference_mode():
            ...
            # Otherwise an r5 policy miss could hide the preflight rejection
            ...
            del x
            torch.cuda.empty_cache()
            _candidate_performance_gate(chunk_kda, counts, c1, result, args.json)
    finally:
        try:
            restoration = _restore_production_map(production_map, production_snapshot)
            candidate_map_result = result["candidate_map"]
            if not isinstance(candidate_map_result, dict):
                raise TypeError("candidate map result schema corrupted during restoration")
            candidate_map_result["restoration"] = restoration
            candidate_map_result["restored"] = True
        finally:
            c1.chunk_kda, pinned.chunk_kda = originals["c1"], originals["pinned"]
        # Failure checkpoints are deliberately also restored: completion is
        # written only after this finally has proven the production map again.
        _write(args.json, result)
    candidate_map_result = result["candidate_map"]
    if not isinstance(candidate_map_result, dict) or candidate_map_result.get("restored") is not True:
        raise RuntimeError("candidate map restoration gate did not pass")
    result["complete"] = True
    _write(args.json, result)
    print(f"wrote diagnostic-only {args.json}; positive={len(result['positive_results'])}, negative={len(result['negative_results'])}")
