#!/usr/bin/env bash
# Clean single-B300 public-FLA packed-varlen integration audit.  No build path.
set -Eeuo pipefail

if [[ "${C1_VARLEN_FLA_INTEGRATION_GPU_AUTHORIZED:-0}" != 1 || "${1:-}" != "--authorized-by-parent" ]]; then
    echo "refusing GPU run: set C1_VARLEN_FLA_INTEGRATION_GPU_AUTHORIZED=1 and pass --authorized-by-parent" >&2
    exit 64
fi
shift
...
EXPECTED_RUNNER_SHA256="5db71f29335220496ca9540924e17c5f160b0bc8237060921cffaecb708f22bb"
EXPECTED_ANALYZER_SHA256="83020b73606b119a6efddadaf621630ed9ac2f23bebd72d7d935c1492ec4e057"
...
mkdir -p "$RESULTS_DIR"
LOG="$RESULTS_DIR/c1_varlen_fla_integration_${LABEL}_job${SLURM_JOB_ID:-none}.log"
exec > >(tee "$LOG") 2>&1
...
require_sha "$OWNED/run_varlen_fla_integration.py" "$EXPECTED_RUNNER_SHA256" "integration runner"
require_sha "$OWNED/analyze_varlen_fla_integration.py" "$EXPECTED_ANALYZER_SHA256" "integration analyzer"
...
sha256sum "$OWNED/run_varlen_fla_integration.py" "$OWNED/analyze_varlen_fla_integration.py" "$OWNED/run_clean_varlen_fla_integration_audit.sh" "$A02_ROOT/team/c1_flashkda/challenge_tp8_dispatch/auto_dispatch.py" "$A02_ROOT/team/c1_flashkda/challenge_tp8_dispatch/fla_backend.py" "$A02_ROOT/team/c1_flashkda/challenge_tp8_dispatch/varlen_metadata.py" "$OWNED/run_varlen_dispatch_confirmation.py" "$C1_PINNED_REFERENCE_HELPER_PATH" "${SO_PATHS[0]}" "$PATCHED_ROOT/flash_kda/__init__.py" "$REFERENCE_ROOT/tests/torch_ref.py" "$FLA_ROOT/fla/__init__.py" "$FLA_ROOT/fla/ops/backends/__init__.py" "$FLA_ROOT/fla/ops/kda/__init__.py" "$FLA_ROOT/fla/ops/kda/backends/__init__.py" "$FLA_ROOT/fla/ops/kda/backends/flash_kda.py" "$FLA_ROOT/fla/ops/kda/chunk.py"
echo "===== PYTHON_COMPILE_GATE ====="; "$PYTHON_BIN" -m py_compile "$OWNED/run_varlen_fla_integration.py"
...
export C1_VARLEN_FLA_INTEGRATION_CLEAN_GPU=1 C1_B300_FLASH_KDA=1 C1_B300_VARLEN_CPU_DESCRIPTOR=1 FLA_FLASH_KDA=1
export C1_VARLEN_FLA_INTEGRATION_RUNNER_SHA256="$EXPECTED_RUNNER_SHA256"
cd "$PATCHED_ROOT"
echo "===== VARLEN_FLA_INTEGRATION ====="
"$PYTHON_BIN" "$OWNED/run_varlen_fla_integration.py" --reference-root "$REFERENCE_ROOT" --json "$RESULTS_DIR/c1_varlen_fla_integration_${LABEL}.json"
require_clean AFTER_INTEGRATION || exit 93
