#!/usr/bin/env python3
"""B300 public-FLA integration audit for CPU-authoritative packed varlen.
...
CLEAN_GPU_GATE_ENV = "C1_VARLEN_FLA_INTEGRATION_CLEAN_GPU"
RUNNER_SHA256_ENV = "C1_VARLEN_FLA_INTEGRATION_RUNNER_SHA256"
...
RUNTIME_DEPENDENCY_SHA256 = {
    ...
    "fla_backend": "6321b1a75713560d25fd92bb94e8e4e15401d206269a8fc10ca5b8ab4433174f",
    "varlen_metadata": "16c01cfc2a8aeee4d80362435053009c3b6397ab09e01d390ac14a38a29b822d",
...
# intersection, not the broader raw-ABI confirmation matrix.  Keeping this
# map literal makes a scope expansion fail closed during both planning and GPU
# execution.
...
def _initial_result(args: argparse.Namespace) -> dict[str, object]:
    return {
        "schema_version": 3,
        "purpose": "public FLA packed-varlen C1/pinned integration audit; no policy mutation",
        "seed": args.seed,
        "positive_cells": [
...
def _positive_cell(cell: Cell, x: object, cpu: torch.Tensor, gpu: torch.Tensor, originals: Mapping[str, Callable[..., Any]], counts: Mapping[str, int], public_fn: Callable[..., Any], c1: object, pinned: object, torch_ref: Callable[..., None], seed: int) -> dict[str, object]:
    ...
    with torch.inference_mode():
        ...
        public_snapshot = _snapshot_input_tensors(x, gpu, cpu, public_initial)
        public, spy = _spy_public(public_fn, x, public_initial, final, gpu, cpu, counts, True, cell.key)
        public_immutability = _assert_input_immutability(f"{cell.key}/public", public_snapshot, x, gpu, cpu, public_initial)
    ...
    return {
        ...
        "input_immutability_fields": sorted({field for evidence in (pinned_immutability, direct_immutability, public_immutability) for field in evidence["fields"]}),
        "input_immutability_by_path": {"pinned": pinned_immutability, "direct_c1": direct_immutability, "public_c1": public_immutability},
...
def _cpu_construction_check() -> dict[str, object]:
    ...
    return {"offsets": list(facts.offsets), "sequence_count": facts.sequence_count, "total_tokens": facts.total_tokens, "passed": True}
...
def _performance_release(public_fn: Callable[..., Any], counts: Mapping[str, int], result: dict[str, object], json_path: Path) -> None:
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("policy fallback classifications must be the frozen 8/1/1 r5 split")
    ...
    try:
        ...
        with torch.inference_mode():
            ...
            del x
            torch.cuda.empty_cache()
            _performance_release(chunk_kda, counts, result, args.json)
    finally:
