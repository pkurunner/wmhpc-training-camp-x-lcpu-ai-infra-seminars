#!/usr/bin/env python3
"""Fail-closed independent audit of the diagnostic-only handoff candidate.
...
summaries and would-pass lists only as claims to cross-check, never as evidence.
"""
...
INTEGRATION_RUNNER_SHA256 = "e07481e7e66551f1f9eaafa7f1fb9db9ef193f1694ff831ee0a07802c8ffbe14"
INTEGRATION_RUNNER_SHA256_ENV = "C1_VARLEN_FLA_HANDOFF_CANDIDATE_RUNNER_SHA256"
...
# Frozen r4 candidate map: installed only within the runner process after the
# r5 production two-cell map has been proven and before it is restored.
POSITIVE: dict[str, str] = {
    "equal_n2_h12_t2048/none": "vshard4_p2",
    "equal_n2_h12_t2048/fp32_final_only": "vshard4_p2",
    "equal_n2_h12_t2048/fp32_both": "vshard4_p2",
    "equal_n4_h12_t2048/none": "vshard2_p2",
    "equal_n4_h12_t2048/fp32_final_only": "vshard2_p2",
    "mixed_n6_h12_t8192/none": "vshard2_p2",
    "mixed_n6_h12_t8192/fp32_final_only": "vshard2_p2",
    "skew_n6_h12_t12288/none": "vshard2_p2",
    "skew_n6_h12_t12288/fp32_final_only": "vshard2_p2",
    "skew_n6_h12_t12288/fp32_both": "vshard4_p2",
}
PRODUCTION_POSITIVE: dict[str, str] = {
    "skew_n6_h12_t12288/none": "vshard2_p2",
    "skew_n6_h12_t12288/fp32_final_only": "vshard2_p2",
}
...
def expected_map_behavior(entries: Mapping[str, str]) -> dict[str, dict[str, str]]:
    """Recreate exact torch-free map lookup evidence emitted by the runner."""

    expected: dict[str, dict[str, str]] = {}
    for cell, variant in entries.items():
        case, contract = cell.split("/", 1)
        offsets = OFFSETS_BY_CASE[case]
        expected[f"{','.join(map(str, offsets))}/{contract}"] = {
            "selected_variant": variant,
            "reason": f"varlen_{CASE_LAYOUTS[case]}_{contract}_whitelist_hit",
        }
    return expected


def require_map_behavior(value: object, expected: Mapping[str, Mapping[str, str]], label: str) -> None:
    actual = mapping(value, label)
    require(set(actual) == set(expected), f"{label}: map behavior key-set drift")
    for key, expected_entry in expected.items():
        require(
            dict(mapping(actual.get(key), f"{label}.{key}")) == dict(expected_entry),
            f"{label}: map behavior drift at {key}",
        )
...
def require_identity_and_gates(data: Mapping[str, Any]) -> None:
    require(
        len(POSITIVE) == 10
        and len(POLICY_FALLBACK_CELLS) == 2
        and set(POSITIVE).isdisjoint(POLICY_FALLBACK_CELLS)
        and set(POSITIVE).union(POLICY_FALLBACK_CELLS) == set(ALL_PUBLIC_CELLS),
        "candidate public matrix constant drift",
    )
    classifications = [entry["classification"] for entry in POLICY_FALLBACK_CELLS.values()]
    require(
        classifications.count("public_release_failed") == 0
        and classifications.count("raw_release_failed") == 1
        and classifications.count("record_only") == 1,
        "candidate fallback classification scope drift",
    )
    require(data.get("schema_version") == SCHEMA_VERSION, "integration schema version drift")
    require(data.get("diagnostic_only") is True and data.get("no_release_authority") is True, "diagnostic authority flags drift")
    require(data.get("complete") is True, "integration artifact incomplete")
    gates = mapping(data.get("gates"), "gates")
    required = ("scope", "clean_gpu", "device", "extension", "fla_pin", "inference_mode", "registry_spy_identity", "prepare_spy_restored", "python_nvidia_clean")
    ...
    require(
        mapping(gates.get("prepare_spy_restored"), "gates.prepare_spy_restored")
        == {"instance_shadow": False, "descriptor_binding": True, "passed": True},
        "prepare-spy restoration gate drift",
    )
    ...
    require(
        ...
        == {
            "positive_cells": 10,
            "policy_fallback_cells": 2,
            "required_positive_cells": 10,
            "required_policy_fallback_cells": 2,
            "passed": True,
        },
        "candidate public scope gate drift",
    )
    ...
    require("B300" in str(device.get("name", "")).upper(), "not B300")
    require(device.get("capability") == [10, 3] and device.get("multiprocessor_count") == 148, "B300 identity drift")
    ...
    require(
        ...
        and runner_path.replace("\\", "/").rstrip("/").endswith(
            "assignment02/team/c1_flashkda/challenge_varlen_dispatch/run_varlen_fla_handoff_candidate.py"
        ),
    ...
    require(clean.get("passed") is True and clean.get("memory_used_mib") == 0, "python clean-GPU identity drift")
    registry = mapping(data.get("registry"), "registry")
    require(isinstance(registry.get("c1_id"), int) and isinstance(registry.get("pinned_id"), int) and registry["c1_id"] != registry["pinned_id"], "registry identity drift")
...
def require_candidate_map_restoration(data: Mapping[str, Any]) -> None:
    """The candidate map must be process-local and provably restored to r5."""

    candidate = mapping(data.get("candidate_map"), "candidate_map")
    require(
        set(candidate) == {"production_expected", "candidate_cells", "installed", "restored", "install", "restoration"},
        "candidate map result field-set drift",
    )
    require(candidate.get("installed") is True and candidate.get("restored") is True, "candidate map lifecycle incomplete")
    require(
        dict(mapping(candidate.get("production_expected"), "candidate_map.production_expected"))
        == PRODUCTION_POSITIVE,
        "candidate map production r5 baseline drift",
    )
    expected_cells = [
        {
            "offsets": OFFSETS_BY_CASE[key.split("/", 1)[0]],
            "contract": key.split("/", 1)[1],
            "variant": variant,
        }
        for key, variant in POSITIVE.items()
    ]
    require(sequence(candidate.get("candidate_cells"), "candidate_map.candidate_cells") == expected_cells, "candidate map exact r4 entries drift")
    install = mapping(candidate.get("install"), "candidate_map.install")
    require(
        set(install) == {"production_map_object_id", "production_before", "candidate_installed", "passed"}
        and install.get("passed") is True
        and type(install.get("production_map_object_id")) is int
        and install.get("production_map_object_id") > 0,
        "candidate map installation evidence drift",
    )
    before = mapping(install.get("production_before"), "candidate_map.install.production_before")
    installed = mapping(install.get("candidate_installed"), "candidate_map.install.candidate_installed")
    require(
        set(before) == {"map_object_id", "entries", "behavior", "passed"}
        and type(before.get("map_object_id")) is int
        and before.get("map_object_id") > 0
        and type(before.get("entries")) is int
        and before.get("entries") == 2
        and before.get("passed") is True,
        "candidate map production-before evidence drift",
    )
    require_map_behavior(before.get("behavior"), expected_map_behavior(PRODUCTION_POSITIVE), "candidate_map.install.production_before.behavior")
    require(
        set(installed) == {"map_object_id", "entries", "behavior", "passed"}
        and type(installed.get("map_object_id")) is int
        and installed.get("map_object_id") > 0
        and type(installed.get("entries")) is int
        and installed.get("entries") == 10
        and installed.get("passed") is True,
        "candidate map installed evidence drift",
    )
    require_map_behavior(installed.get("behavior"), expected_map_behavior(POSITIVE), "candidate_map.install.candidate_installed.behavior")
    require(
        before.get("map_object_id") == install.get("production_map_object_id") == installed.get("map_object_id"),
        "candidate map object identity changed during install",
    )
    restoration = mapping(candidate.get("restoration"), "candidate_map.restoration")
    require(
        set(restoration) == {"map_object_id", "entries", "behavior", "candidate_cell_after_restore", "passed"}
        and type(restoration.get("map_object_id")) is int
        and restoration.get("map_object_id") > 0
        and restoration.get("map_object_id") == install.get("production_map_object_id")
        and type(restoration.get("entries")) is int
        and restoration.get("entries") == 2
        and restoration.get("passed") is True,
        "candidate map restoration identity/content drift",
    )
    restoration_behavior_wrapper = mapping(
        restoration.get("behavior"), "candidate_map.restoration.behavior"
    )
    require(
        set(restoration_behavior_wrapper) == {"map_object_id", "entries", "behavior", "passed"}
        and type(restoration_behavior_wrapper.get("map_object_id")) is int
        and restoration_behavior_wrapper.get("map_object_id") > 0
        and restoration_behavior_wrapper.get("map_object_id") == restoration.get("map_object_id")
        and type(restoration_behavior_wrapper.get("entries")) is int
        and restoration_behavior_wrapper.get("entries") == restoration.get("entries")
        and restoration_behavior_wrapper.get("passed") is True,
        "candidate map restoration behavior-wrapper identity/content drift",
    )
    require_map_behavior(
        restoration_behavior_wrapper.get("behavior"),
        expected_map_behavior(PRODUCTION_POSITIVE),
        "candidate_map.restoration.behavior.behavior",
    )
    rejected = mapping(restoration.get("candidate_cell_after_restore"), "candidate_map.candidate_cell_after_restore")
    require(
        rejected == {
            "selected_variant": None,
            "reason": "varlen_equal_n2_h12_t4096_none_not_whitelisted",
        },
        "candidate map restoration behavior drift",
    )


def require_positive(data: Mapping[str, Any]) -> dict[str, object]:
    preregistered = sequence(data.get("positive_cells"), "positive_cells")
    require(len(preregistered) == 10, "positive preregistration count drift")
    ...
    comparisons = (
        "pinned_vs_torch_ref", "direct_c1_vs_pinned", "public_vs_pinned",
    ...
    for key, expected_variant in POSITIVE.items():
        ...
        # snapshots for all three routes, an offline JSON auditor cannot
        ...
        require(
            ...
            and isinstance(c1_handoff.get("prepare_calls_total"), int)
        ...
        require(
            ...
            and isinstance(pinned_handoff.get("prepare_calls_total"), int)
...
def require_skew_none_decision(value: object, label: str, *, cache_hit: bool) -> None:
    ...
    check: it is also evidence that the public route selected the r5-released
...
def require_performance(data: Mapping[str, Any]) -> tuple[dict[str, object], list[str], list[str]]:
    ...
    require(
        set(performance) == {"pre_registered", "cold_miss_observations", "cells", "would_pass_cells", "failed_cells", "complete"},
        "candidate performance field-set drift",
    )
    ...
    require(
        set(prereg) == {
            "cells", "fixed_measurement_seed", "paths", "repeats", "warmup_per_path_per_repeat",
            "cyclic_cuda_event_samples_per_path_per_repeat", "percentiles", "minimum_c1_margin_over_pinned",
            "candidate_rule", "timing_contract", "timed_public_calls", "warmup_public_calls",
            "timed_event_synchronizations", "one_hour_mean_call_budget_ms_including_warmup",
            "prepare_spy_in_timed_region",
        },
        "candidate performance preregistration field-set drift",
    )
    ...
    require(
        prereg.get("candidate_rule")
        == "A candidate cell would pass only if both repeats have C1 as P50/P95/P99 winner with every margin >=2%; this diagnostic never changes a release policy."
        and prereg.get("prepare_spy_in_timed_region") is False,
        "candidate performance authority/spy scope drift",
    )
    ...
    for key, expected_variant in POSITIVE.items():
        ...
        require(
            set(cell) == {"expected_winner", "repeats", "input_immutability_exact", "input_immutability_fields", "would_pass"}
            and cell.get("expected_winner") == "public_registry_c1",
            f"performance candidate cell field/winner drift: {key}",
        )
        ...
        require(cell.get("would_pass") is passing, f"candidate would-pass bool drift: {key}")
        if passing:
            released.append(key)
        else:
            failed.append(key)
        audit[key] = {"expected_variant": expected_variant, "repeats": rows, "would_pass": passing}
    require(performance.get("would_pass_cells") == released, "nested candidate would-pass list drift")
    require(performance.get("failed_cells") == failed, "nested performance failed list drift")
    require(data.get("would_pass_cells") == released, "top-level candidate would-pass list drift")
    require(data.get("failed_cells") == failed, "top-level candidate failed list drift")
...
def audit(path: Path, expected_sha256: str) -> dict[str, object]:
    ...
    require_candidate_map_restoration(data)
    ...
    return {
        ...
        "independently_would_pass_cells": released,
        "independently_failed_cells": failed,
    }
...
def main() -> None:
    ...
    parser.add_argument("artifact", type=Path, help="completed diagnostic-only handoff candidate JSON")
