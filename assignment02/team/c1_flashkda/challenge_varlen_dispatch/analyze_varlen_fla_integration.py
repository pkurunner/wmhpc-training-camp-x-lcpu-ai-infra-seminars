#!/usr/bin/env python3
"""Fail-closed independent audit of the public-FLA packed-varlen experiment.
...
INTEGRATION_RUNNER_SHA256_ENV = "C1_VARLEN_FLA_INTEGRATION_RUNNER_SHA256"
...
INTEGRATION_RUNTIME_IDENTITIES = {
    ...
    "fla_backend": (
        "6321b1a75713560d25fd92bb94e8e4e15401d206269a8fc10ca5b8ab4433174f",
        "assignment02/team/c1_flashkda/challenge_tp8_dispatch/fla_backend.py",
    ),
    "varlen_metadata": (
        "16c01cfc2a8aeee4d80362435053009c3b6397ab09e01d390ac14a38a29b822d",
...
# r5 production freeze: only the public performance-release intersection may
...
def require_identity_and_gates(data: Mapping[str, Any]) -> None:
    require(
        ...
        "r5 public matrix constant drift",
    )
    ...
    require(
        ...
        "r5 fallback classification scope drift",
    )
    ...
    required = ("scope", "clean_gpu", "device", "extension", "fla_pin", "inference_mode", "registry_spy_identity", "python_nvidia_clean")
    ...
    require(
        ...
        and runner_path.replace("\\", "/").rstrip("/").endswith(
            "assignment02/team/c1_flashkda/challenge_varlen_dispatch/run_varlen_fla_integration.py"
        ),
...
def require_positive(data: Mapping[str, Any]) -> dict[str, object]:
    ...
    comparisons = ("pinned_vs_torch_ref", "direct_c1_vs_pinned", "public_vs_pinned", "direct_c1_vs_torch_ref", "public_vs_torch_ref")
    for key, expected_variant in POSITIVE.items():
        ...
        require(set(paths) == {"pinned", "direct_c1", "public_c1"}, f"positive {key}: immutable-input route scope drift")
