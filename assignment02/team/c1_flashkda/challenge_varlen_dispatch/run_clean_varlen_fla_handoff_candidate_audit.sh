#!/usr/bin/env bash
# Clean single-B300 diagnostic-only public-FLA handoff candidate gate.  No build path.
set -Eeuo pipefail

if [[ "${C1_VARLEN_FLA_HANDOFF_CANDIDATE_GPU_AUTHORIZED:-0}" != 1 || "${1:-}" != "--authorized-by-parent" ]]; then
    echo "refusing GPU run: set C1_VARLEN_FLA_HANDOFF_CANDIDATE_GPU_AUTHORIZED=1 and pass --authorized-by-parent" >&2
    exit 64
fi
shift
...
EXPECTED_RUNNER_SHA256="e07481e7e66551f1f9eaafa7f1fb9db9ef193f1694ff831ee0a07802c8ffbe14"
EXPECTED_ANALYZER_SHA256="10e3ebd2d788f8bdc4ebf5b78da4e285acc8ec680d012650b6af7e7ff4c32481"
...
mkdir -p "$RESULTS_DIR"
LOG="$RESULTS_DIR/c1_varlen_fla_handoff_candidate_${LABEL}_job${SLURM_JOB_ID:-none}.log"
exec > >(tee "$LOG") 2>&1
...
require_sha "$OWNED/run_varlen_fla_handoff_candidate.py" "$EXPECTED_RUNNER_SHA256" "handoff candidate runner"
require_sha "$OWNED/analyze_varlen_fla_handoff_candidate.py" "$EXPECTED_ANALYZER_SHA256" "handoff candidate analyzer"
...
sha256sum "$OWNED/run_varlen_fla_handoff_candidate.py" "$OWNED/analyze_varlen_fla_handoff_candidate.py" "$OWNED/run_clean_varlen_fla_handoff_candidate_audit.sh" "$A02_ROOT/team/c1_flashkda/challenge_tp8_dispatch/auto_dispatch.py" "$A02_ROOT/team/c1_flashkda/challenge_tp8_dispatch/fla_backend.py" "$A02_ROOT/team/c1_flashkda/challenge_tp8_dispatch/varlen_metadata.py" "$A02_ROOT/team/c1_flashkda/challenge_tp8_dispatch/test_auto_dispatch_policy.py" "$A02_ROOT/team/c1_flashkda/challenge_tp8_dispatch/test_varlen_metadata_policy.py" "$OWNED/run_varlen_dispatch_confirmation.py" "$C1_PINNED_REFERENCE_HELPER_PATH" "${SO_PATHS[0]}" "$PATCHED_ROOT/flash_kda/__init__.py" "$REFERENCE_ROOT/tests/torch_ref.py" "$FLA_ROOT/fla/__init__.py" "$FLA_ROOT/fla/ops/backends/__init__.py" "$FLA_ROOT/fla/ops/kda/__init__.py" "$FLA_ROOT/fla/ops/kda/backends/__init__.py" "$FLA_ROOT/fla/ops/kda/backends/flash_kda.py" "$FLA_ROOT/fla/ops/kda/chunk.py"
echo "===== PYTHON_COMPILE_GATE ====="; "$PYTHON_BIN" -m py_compile "$OWNED/run_varlen_fla_handoff_candidate.py" "$OWNED/analyze_varlen_fla_handoff_candidate.py"
...
export C1_VARLEN_FLA_HANDOFF_CANDIDATE_CLEAN_GPU=1 C1_B300_FLASH_KDA=1 C1_B300_VARLEN_CPU_DESCRIPTOR=1 FLA_FLASH_KDA=1
export C1_VARLEN_FLA_HANDOFF_CANDIDATE_RUNNER_SHA256="$EXPECTED_RUNNER_SHA256"
cd "$PATCHED_ROOT"
echo "===== VARLEN_FLA_HANDOFF_CANDIDATE ====="
ARTIFACT="$RESULTS_DIR/c1_varlen_fla_handoff_candidate_${LABEL}.json"
AUDIT="$RESULTS_DIR/c1_varlen_fla_handoff_candidate_${LABEL}.independent_audit.json"
"$PYTHON_BIN" "$OWNED/run_varlen_fla_handoff_candidate.py" --reference-root "$REFERENCE_ROOT" --json "$ARTIFACT"
require_clean AFTER_CANDIDATE || exit 93
ARTIFACT_SHA256="$(sha256sum "$ARTIFACT" | awk '{print $1}')"
echo "===== INDEPENDENT_AUDIT ====="
"$PYTHON_BIN" "$OWNED/analyze_varlen_fla_handoff_candidate.py" "$ARTIFACT" --expected-sha256 "$ARTIFACT_SHA256" --json "$AUDIT"
