#!/usr/bin/env bash
# Fresh, candidate-only SM103a build; it never imports the extension or uses CUDA.
set -Eeuo pipefail

if [[ "${C1_PHASE1PF_BUILD_AUTHORIZED:-0}" != 1 || "${1:-}" != "--authorized-by-parent" ]]; then
    echo "refusing build: set C1_PHASE1PF_BUILD_AUTHORIZED=1 and pass --authorized-by-parent" >&2
    exit 64
fi
shift

: "${SOURCE_BASE:?set SOURCE_BASE to clean FlashKDA 1ce47ea source}"
: "${SOURCE_FRESH:?set SOURCE_FRESH to new non-existent candidate destination}"
...
: "${BUILD_LOG:?set BUILD_LOG to ptxas-preserving build log}"
: "${RESOURCE_LOG:?set RESOURCE_LOG to CUBIN resource output}"
: "${PTXAS_JSON:?set PTXAS_JSON to resource-audit JSON output}"
SUPPORT_BIN="${SUPPORT_BIN:?set SUPPORT_BIN to helper directory}"
...
GENERATOR="$OWNED/apply_phase1_fragment_prefetch_patch.py"
...
export FLASH_KDA_CUDA_ARCHS=103a NVCC_THREADS="${NVCC_THREADS:-8}" MAX_JOBS="${MAX_JOBS:-4}"
...
sha256sum "${extensions[0]}" csrc/flash_kda.cpp csrc/fwd.h csrc/smxx/fwd_launch.cu \
  ...
  csrc/smxx/fwd_kernel2_vshard4_p2_phase1pf.cuh "$RESOURCE_LOG"
"$PYTHON_BIN" "$OWNED/ptxas_audit.py" "$BUILD_LOG" --resource-log "$RESOURCE_LOG" --json "$PTXAS_JSON" \
  --require-formal-bf16-fixed-zero-spill --require-shared-memory-evidence
git status --short
date -Is
