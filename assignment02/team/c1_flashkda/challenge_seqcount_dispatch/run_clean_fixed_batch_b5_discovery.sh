#!/usr/bin/env bash
# One-allocation, discovery-only B=5 raw-wrapper experiment on a clean B300.
...
if [[ "${C1_FIXED_BATCH_B5_DISCOVERY_GPU_AUTHORIZED:-0}" != 1 || "${1:-}" != "--authorized-by-parent" ]]; then
    echo "refusing GPU run: set C1_FIXED_BATCH_B5_DISCOVERY_GPU_AUTHORIZED=1 and pass --authorized-by-parent" >&2
    exit 64
fi
shift
...
EXPECTED_ANALYZER_SHA256="7f93cb81dd76263e6eeb8017872636fd0c636578d975a1f2580f845c61f1735d"
...
mkdir -p "$RESULTS_DIR"
LOG="$RESULTS_DIR/c1_fixed_batch_b5_discovery_${LABEL}_job${SLURM_JOB_ID:-none}.log"
exec > >(tee "$LOG") 2>&1
...
require_sha "$OWNED/analyze_fixed_batch_b5_discovery.py" "$EXPECTED_ANALYZER_SHA256" "B=5 analyzer"
...
"$PYTHON_BIN" "$OWNED/run_fixed_batch_b5_discovery.py" \
    ...
    --json "$RESULTS_DIR/c1_fixed_batch_b5_discovery_${LABEL}.plan.json"
...
run_main() {
    local index="$1" artifact="$RESULTS_DIR/c1_fixed_batch_b5_discovery_${LABEL}_main${1}.json"
    echo "===== B5_MAIN_${index} ====="; date -Is
...
main0="$RESULTS_DIR/c1_fixed_batch_b5_discovery_${LABEL}_main0.json"
main1="$RESULTS_DIR/c1_fixed_batch_b5_discovery_${LABEL}_main1.json"
AUDIT="$RESULTS_DIR/c1_fixed_batch_b5_discovery_${LABEL}.independent_audit.json"
