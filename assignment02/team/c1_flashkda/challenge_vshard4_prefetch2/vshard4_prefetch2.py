"""ABI wrapper for the isolated V=32, four-CTA/head P2S3 extension entry."""
...
def fwd(
    ...
    """Run the proven-only ``K=128,V_s=32,PREFETCH=2,S=3`` K2 variant."""
    try:
        from flash_kda_C import fwd_vshard4_p2, get_workspace_size
    except ImportError as exc:  # pragma: no cover - target-host diagnostic
        raise RuntimeError(
            "fwd_vshard4_p2 is unavailable; generate the one-shot vshard4 P2S3 "
            "extension in a fresh FlashKDA 1ce47ea worktree first"
        ) from exc
    if q.ndim != 4 or q.shape[-1] != 128 or any(t.shape != q.shape for t in (k, v, g, out)):
        raise ValueError("vshard4_p2 requires identical [B,T,H,128] q/k/v/g/out tensors")
