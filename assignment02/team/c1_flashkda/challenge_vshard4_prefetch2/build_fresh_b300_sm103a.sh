#!/usr/bin/env bash
# Fresh CPU-side SM103a build. It never imports the extension or launches a kernel.
set -Eeuo pipefail

if [[ "${C1_VSHARD4_P2_BUILD_AUTHORIZED:-0}" != 1 || "${1:-}" != "--authorized-by-parent" ]]; then
    echo "refusing build: set C1_VSHARD4_P2_BUILD_AUTHORIZED=1 and pass --authorized-by-parent" >&2
    exit 64
fi
shift
...
GENERATOR="$OWNED/apply_vshard4_prefetch2_patch.py"
...
"$PYTHON_BIN" "$OWNED/ptxas_audit.py" "$BUILD_LOG" --json "$PTXAS_JSON" \
  --require-vshard4-p2-bf16-fixed-zero-spill
git status --short
date -Is
