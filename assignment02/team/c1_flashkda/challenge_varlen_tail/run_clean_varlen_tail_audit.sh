#!/usr/bin/env bash
# Clean B300 correctness/performance audit for tails, batches, and varlen.
set -Eeuo pipefail

if [[ "${C1_VARLEN_TAIL_GPU_AUTHORIZED:-0}" != 1 || "${1:-}" != "--authorized-by-parent" ]]; then
    echo "refusing GPU run: set C1_VARLEN_TAIL_GPU_AUTHORIZED=1 and pass --authorized-by-parent" >&2
    exit 64
fi
shift
...
DISPATCH="$A02_ROOT/team/c1_flashkda/challenge_tp8_dispatch"
RESULTS_DIR="${RESULTS_DIR:-$OWNED/results}"
mkdir -p "$RESULTS_DIR"
LOG="$RESULTS_DIR/c1_varlen_tail_${LABEL}_job${SLURM_JOB_ID:-none}.log"
exec > >(tee "$LOG") 2>&1
...
sha256sum \
    ...
    "$OWNED/run_clean_varlen_tail_audit.sh" \
    "$DISPATCH/auto_dispatch.py" \
...
cd "$PATCHED_ROOT"
echo "===== VARLEN_BATCH_TAIL_MATRIX ====="
"$PYTHON_BIN" "$OWNED/run_varlen_tail.py" \
    --reference-root "$REFERENCE_ROOT" --warmup 30 --samples 300 \
    --json "$RESULTS_DIR/c1_varlen_tail_${LABEL}.json"
