...
if queue_state=$(squeue -h -j "${C2_NCU_JOB_ID}" -o '%i' 2>&1); then
  [[ -z "${queue_state}" ]]
else
  [[ "${queue_state}" == 'slurm_load_jobs error: Invalid job id specified' ]]
fi
if scheduler_state=$(scontrol show job -o "${C2_NCU_JOB_ID}" 2>&1); then
  [[ -n "${scheduler_state}" ]]
  ...
  # This cluster can purge a completed job from slurmctld before slurmdbd is
  # reachable.  Accept only that exact controller response; the immutable
  # job final-status, sidecar, outputs manifest, result schema, UUID, and
  # clean-GPU gates below must still all pass before any archive is published.
