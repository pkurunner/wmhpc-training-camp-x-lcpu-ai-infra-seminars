...
def load_base(path: Path):
    spec = importlib.util.spec_from_file_location("c2_v6_worker_base", path)
...
def exercise(base: Any, op: Any, inputs: tuple[torch.Tensor, ...], contract: Any, seed: int) -> dict[str, Any]:
    ...
    all_gates = bool(finite and allclose and output.data_ptr() == pointer_before and returned is None
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if fixture.get("schema") != "c2-native-v6-fixed-fixtures-v1":
    ...
    return {
        "schema": "c2-native-plugin-v6-fixed-fixture-worker-v1", "role": args.role,
...
def main() -> int:
    ...
    try:
        ...
        payload = {"schema": "c2-native-plugin-v6-fixed-fixture-worker-v1", "role": args.role,
