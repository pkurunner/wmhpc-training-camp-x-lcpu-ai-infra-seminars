#!/usr/bin/env bash
# Explicitly authorized, frozen prepared stage-5 versus stage-3 AB/BA audit.
# This wrapper never submits or allocates Slurm resources.  It only runs inside
# a coordinator-provided allocation after confirming that the GPU is empty.

set -Eeuo pipefail

if [[ "${C2_PREPARED_STAGE5_ABBA_AUTHORIZED:-}" != "1" || "${1:-}" != "--authorized-by-parent" ]]; then
    printf '%s\n' 'Refusing performance run without explicit coordinator authorization.' >&2
    exit 64
fi
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    printf '%s\n' 'Refusing performance run outside a Slurm allocation (SLURM_JOB_ID is unset).' >&2
    exit 64
fi
...
out_dir="${C2_PREPARED_STAGE5_ABBA_OUT_DIR:-${c2_root}/experiment_logs/prepared_stage5_abba}"
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
audit_log="${out_dir}/c2_prepared_stage5_abba_clean_audit_${stamp}.log"
audit_batches="${C2_PREPARED_STAGE5_ABBA_BATCHES:-1,16}"

if [[ ! -x "${python_bin}" ]]; then
    printf 'Missing explicitly selected Python: %s\n' "${python_bin}" >&2
    exit 65
fi
# The machine-readable validator below deliberately uses Python assertions.
# Pin optimization off and prove that they are live before collecting evidence.
...
case "${audit_batches}" in
    1|16|1,16) ;;
    *) printf 'C2_PREPARED_STAGE5_ABBA_BATCHES must be 1,16 (or one member); got %q\n' "${audit_batches}" >&2; exit 66 ;;
esac
...
validate_json() {
    ...
    "${python_bin}" - "${json_path}" "${batch}" "${c2_root}" <<'PY'
import hashlib
import json
import math
import statistics
...
path, batch_text, c2_root_text = sys.argv[1:]
...
print(json.dumps({
    "json": path, "batch": batch, "control_us": control_us, "candidate_us": candidate_us,
...
run_batch() {
    local batch="$1"
    local json_path="${out_dir}/c2_prepared_stage5_abba_b${batch}_bf16_c1_clean_${stamp}.json"
    local stdout_path="${out_dir}/c2_prepared_stage5_abba_b${batch}_bf16_c1_clean_${stamp}.stdout.log"
    printf '\n===== frozen prepared ABBA batch B=%s =====\n' "${batch}"
    require_empty_gpu "before B=${batch}" || return $?

    local cli_rc=0
    set +e
    (
        cd "${c2_root}"
        PYTHONPATH=. "${python_bin}" -m challenge_v2.prepared_stage_abba_cli \
            --batch "${batch}" --seed 20260828 --max-seq-len 4096 \
            --require-strict-10 --output "${json_path}"
    ) >"${stdout_path}" 2>&1
    cli_rc=$?
    set -e
    if [[ "${cli_rc}" -ne 0 && "${cli_rc}" -ne 3 ]]; then
        printf 'ABORT: stage-5 ABBA CLI failed before valid JSON (rc=%s): %s\n' "${cli_rc}" "${stdout_path}" >&2
        return "${cli_rc}"
    fi
    # run_batch is called in an if condition, where bash suppresses errexit.
    # Make every evidence gate explicitly propagate its own failure.
    validate_json "${json_path}" "${batch}" || return $?
    require_empty_gpu "after B=${batch}" || return $?
    return "${cli_rc}"
}
...
for batch in "${requested_batches[@]}"; do
    if run_batch "${batch}"; then
        ...
        if [[ "${rc}" -eq 3 ]]; then
            strict_failure=1
            printf 'STRICT-10 GATE FAILED for B=%s; retaining validated evidence and continuing.\n' "${batch}" >&2
        else
            exit "${rc}"
        fi
    fi
...
printf '\nPrepared stage-5 fixed AB/BA audit completed: %s\n' "${audit_log}"
