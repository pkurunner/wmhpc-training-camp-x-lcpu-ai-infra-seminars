...
// The observed two-seed BF16-output envelope is below 1.6e-4 absolute error.
// Keep enough headroom for BF16 partial/output rounding while rejecting subtle
// online-softmax or partial-merge regressions that the former 3e-2 gate could hide.
...
constexpr const char* kMBarrierPhase =
    "pending in this control: a separately audited variant exercises cluster-scope PTX mbarrier; "
    "two cooperative_groups::cluster_group::sync phases protect this DSM handoff";
...
// in its own CTA-local shared memory.  There is no global partial buffer.
__global__ void cluster_attention_kernel(const __nv_bfloat16* query,
  ...
  // Every CTA, including role 3, follows the same two cluster-sync lifecycle.
  __syncthreads();
  cluster.sync();
...
SeedResult run_seed(int seed) {
  ...
  try {
    ...
    CUDA_CHECK(cudaLaunchKernelEx(&launch_config, cluster_attention_kernel,
...
void print_success_json(const cudaDeviceProp& property,
  ...
  std::cout << std::setprecision(9)
            << "{\"schema\":\"c2-cluster-attention-smoke-v1\","
            << "\"status\":\"pass\",\"boundary\":\"" << json_escape(kBoundary) << "\","
            << "\"mbarrier_pending\":true,\"mbarrier_phase\":\"" << json_escape(kMBarrierPhase) << "\","
            ...
            << "\"tolerance\":{\"rtol\":" << kRtol << ",\"atol\":" << kAtol
             << ",\"reason\":\"BF16 producer partial and caller output are checked against an independent FP64-accumulator oracle\"},"
...
void print_failure_json(const std::string& error) {
  std::cout << "{\"schema\":\"c2-cluster-attention-smoke-v1\",\"status\":\"fail\",\"error\":\""
...
int main() {
  try {
    ...
    CUDA_CHECK(cudaFuncGetAttributes(&attributes, cluster_attention_kernel));
