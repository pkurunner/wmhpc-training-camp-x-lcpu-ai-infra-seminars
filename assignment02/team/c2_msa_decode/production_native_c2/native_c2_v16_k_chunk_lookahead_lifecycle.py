#!/usr/bin/env python3
"""Fail-closed production-lifecycle entry point for the v16 K-chunk-lookahead native-C2 wheel.

The immutable staged lifecycle core performs the real checks.  This reviewed
entry point binds that core to the accepted v16 K-chunk-lookahead wheel and makes
...
_SCHEMA = "c2-native-c2-v16-k-chunk-lookahead-production-lifecycle-v1"
_CORE_SCHEMA = "c2-native-c2-v16-k-chunk-lookahead-lifecycle-core-binding-v1"
_THREAD_COUNT = 8
...
def _load_core(path: Path, expected_sha: str) -> ModuleType:
    ...
    if path.resolve() == Path(__file__).resolve():
        raise RuntimeError("lifecycle core must be distinct from the v16 K-chunk-lookahead entry point")
    ...
    spec = importlib.util.spec_from_file_location("native_c2_v16_k_chunk_lookahead_lifecycle_core", path)
...
def _assert_bounded_memory(stage: dict[str, Any], label: str) -> None:
    ...
    if counters.get("pass") is not True or samples.get("pass") is not True:
        raise RuntimeError(f"v16 K-chunk-lookahead lifecycle {label} memory bound did not pass")
    if counters.get("bound_bytes") != 8 * 1024 * 1024:
        raise RuntimeError(f"v16 K-chunk-lookahead lifecycle {label} used the wrong memory bound")
    ...
    if set(counters.get("checks", {})) != expected_counter_checks or not all(counters["checks"].values()):
        raise RuntimeError(f"v16 K-chunk-lookahead lifecycle {label} CUDA counter checks are incomplete")
    ...
    if set(samples.get("checks", {})) != expected_sample_checks or not all(samples["checks"].values()):
        raise RuntimeError(f"v16 K-chunk-lookahead lifecycle {label} allocation samples are incomplete")
...
def _required_gate_check(result: dict[str, Any]) -> None:
    ...
    if not isinstance(gates, dict) or set(gates) != required or not all(gates.values()):
        raise RuntimeError("one or more mandatory v16 K-chunk-lookahead lifecycle gates did not pass")
    ...
    if first_load.get("thread_count") != _THREAD_COUNT or first_load.get("load_library_call_count") != 1:
        raise RuntimeError("v16 K-chunk-lookahead lifecycle did not prove exactly one loader call across eight threads")
    steady = result.get("steady_lifecycle", {})
    if steady.get("iterations") != _MIN_STEADY_ITERATIONS or steady.get("pass") is not True:
        raise RuntimeError("v16 K-chunk-lookahead lifecycle did not complete 1,000 real forwards")
    _assert_bounded_memory(steady, "steady")
    graph = result.get("cuda_graph", {})
    if graph.get("pass") is not True:
        raise RuntimeError("v16 K-chunk-lookahead lifecycle CUDA Graph gate did not pass")
    for label in ("original_query", "updated_query"):
        ...
        if replays.get("replays") != _MIN_GRAPH_REPLAYS or replays.get("pass") is not True:
            raise RuntimeError(f"v16 K-chunk-lookahead lifecycle did not replay {label} 100 times")
    ...
    if dynamic.get("mode") != "selection_only" or not dynamic.get("rows") or not all(row.get("pass") is True for row in dynamic["rows"]):
        raise RuntimeError("v16 K-chunk-lookahead lifecycle rejection boundary is incomplete")
    ...
    if [row.get("sequence_length") for row in boundaries] != [2048, 2049, 4095, 4096]:
        raise RuntimeError("v16 K-chunk-lookahead lifecycle did not cover the required sequence-length boundaries")
    if not all(row.get("pass") is True for row in boundaries):
        raise RuntimeError("one or more v16 K-chunk-lookahead sequence-length boundaries failed")


def _run(args: argparse.Namespace) -> dict[str, Any]:
    if args.steady_iterations != _MIN_STEADY_ITERATIONS:
        raise ValueError("v16 K-chunk-lookahead lifecycle requires exactly 1,000 steady forwards")
    if args.graph_replays != _MIN_GRAPH_REPLAYS:
        raise ValueError("v16 K-chunk-lookahead lifecycle requires exactly 100 graph replays per static query")
    if args.memory_bound_bytes != 8 * 1024 * 1024:
        raise ValueError("v16 K-chunk-lookahead lifecycle requires an exact 8 MiB CUDA-memory bound")
