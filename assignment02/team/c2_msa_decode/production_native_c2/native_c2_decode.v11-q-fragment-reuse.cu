...
__global__ void native_c2_msa_decode_kernel(
  ...
  if (role < kProducerCtas) {
    ...
    wmma::fragment<wmma::matrix_a, kTile, kTile, kTile, __nv_bfloat16,
                   wmma::row_major>
        q_fragment_7;
    wmma::load_matrix_sync(q_fragment_0, &q_tile[0][0 * kTile], kHeadDim);
    wmma::load_matrix_sync(q_fragment_1, &q_tile[0][1 * kTile], kHeadDim);
    wmma::load_matrix_sync(q_fragment_2, &q_tile[0][2 * kTile], kHeadDim);
    wmma::load_matrix_sync(q_fragment_3, &q_tile[0][3 * kTile], kHeadDim);
    wmma::load_matrix_sync(q_fragment_4, &q_tile[0][4 * kTile], kHeadDim);
    wmma::load_matrix_sync(q_fragment_5, &q_tile[0][5 * kTile], kHeadDim);
    wmma::load_matrix_sync(q_fragment_6, &q_tile[0][6 * kTile], kHeadDim);
    wmma::load_matrix_sync(q_fragment_7, &q_tile[0][7 * kTile], kHeadDim);
