#!/usr/bin/env python3
"""Fail-closed production-lifecycle entry point for the v11 native-C2 wheel.

The lifecycle implementation is staged with this entry point as an immutable
reviewed input.  This small entry point owns the v11 result schema and checks
the implementation binding before invoking the real lifecycle checks: eight
simultaneous first loaders, real ``MiniMaxM3SparseMSAImpl.forward`` calls,
1,000 steady forwards, CUDA Graph capture/replay, native-selection and
rejection boundaries, exact sequence-length boundaries, and post-run
dispatcher stability.  It never converts an unsupported shape into a claim
that Triton executed.
"""
...
_SCHEMA = "c2-native-c2-v11-production-lifecycle-v1"
_CORE_SCHEMA = "c2-native-c2-v11-lifecycle-core-binding-v1"
_THREAD_COUNT = 8
...
def _load_core(path: Path, expected_sha: str) -> ModuleType:
    ...
    if path.resolve() == Path(__file__).resolve():
        raise RuntimeError("lifecycle core must be distinct from the v11 entry point")
    ...
    spec = importlib.util.spec_from_file_location("native_c2_v11_lifecycle_core", path)
    ...
    # The staged implementation is deliberately schema-parametric; setting this
    # before it runs makes every success and failure result unambiguously v11.
...
def _required_gate_check(result: dict[str, Any]) -> None:
    ...
    if not isinstance(gates, dict) or set(gates) != required or not all(gates.values()):
        raise RuntimeError("one or more mandatory v11 lifecycle gates did not pass")
    ...
    if first_load.get("thread_count") != _THREAD_COUNT or first_load.get("load_library_call_count") != 1:
        raise RuntimeError("v11 lifecycle did not prove exactly one loader call across eight threads")
    if result.get("steady_lifecycle", {}).get("iterations") != _MIN_STEADY_ITERATIONS:
        raise RuntimeError("v11 lifecycle did not complete 1,000 real forwards")
    graph = result.get("cuda_graph", {})
    if graph.get("original_query", {}).get("replays", {}).get("replays") != _MIN_GRAPH_REPLAYS:
        raise RuntimeError("v11 lifecycle did not replay the original CUDA Graph input 100 times")
    if graph.get("updated_query", {}).get("replays", {}).get("replays") != _MIN_GRAPH_REPLAYS:
        raise RuntimeError("v11 lifecycle did not replay the updated CUDA Graph input 100 times")
    dynamic = result.get("dynamic_rejection_gate", {})
    if dynamic.get("mode") != "selection_only" or not all(row.get("pass") is True for row in dynamic.get("rows", [])):
        raise RuntimeError("v11 lifecycle rejection boundary is incomplete")
    ...
    if [row.get("sequence_length") for row in boundaries] != [2048, 2049, 4095, 4096]:
        raise RuntimeError("v11 lifecycle did not cover the required sequence-length boundaries")
    if not all(row.get("pass") is True for row in boundaries):
        raise RuntimeError("one or more v11 sequence-length boundaries failed")


def _run(args: argparse.Namespace) -> dict[str, Any]:
    if args.steady_iterations != _MIN_STEADY_ITERATIONS:
        raise ValueError("v11 lifecycle requires exactly 1,000 steady forwards")
    if args.graph_replays != _MIN_GRAPH_REPLAYS:
        raise ValueError("v11 lifecycle requires exactly 100 graph replays per static query")
